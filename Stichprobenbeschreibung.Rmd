...
## BigFive
```{r bigFive_graph}
mrg_bigFive <- data_bigFive %>% select(-ResponseId) %>% pivot_longer(cols= c(offenheit, neuro, vertraeglich, extraversion, gewissenhaft), names_to = "Persönlichkeit", values_to = "Ausprägnung")

ggplot(mrg_bigFive, 
       aes(x=reorder(Persönlichkeit, Ausprägnung, FUN = mean), 
           y=Ausprägnung))+
...
```
Bei der Betrachtung der Persönlichkeitsmerkmale der Probanden wird erkennbar, dass die Teilnehmer ganz schwache neurotische Tendenzen zeigen (`r describe.mean.sd(data_bigFive$neuro)`). Darüberhinaus sind die Befragten eher verträgliche (`r describe.mean.sd(data_bigFive$vertraeglich)`), extrovertierte (`r describe.mean.sd(data_bigFive$extraversion)`), offene (`r describe.mean.sd(data_bigFive$offenheit)`), und sehr gewissenhafte (`r describe.mean.sd(data_bigFive$gewissenhaft)`) Menschen.
...
* Bevorzugten Themengebiete: schwach zugeneigt (`r describe.mean.sd(data_publishing$RS_thema)`)
* Bewertungen über gerade gelesene Artikel: sehr schwach abgeneigt (`r describe.mean.sd(data_publishing$RS_gelesene_artikel)`)
* Persönliche Interessen (Hobbys): sehr schwach abgeneigt (`r describe.mean.sd(data_publishing$RS_hobby)`)
* Persönliche Angaben (Geschlecht, Alter): schwach abgeneigt (`r describe.mean.sd(data_publishing$RS_demograph)`)
* Aktueller Standort: abgeneigt (`r describe.mean.sd(data_publishing$RS_standort)`)
* Politische Einstellung: abgeneigt (`r describe.mean.sd(data_publishing$RS_politik)`)
* Religionszugehörigkeit: abgeneigt (`r describe.mean.sd(data_publishing$RS_religion)`)
...
wilcox.test(data_RS_trust$RS_Privacy_Trust ~ data_demograph$gender)
t.test(data_RS_trust$RS_Privacy_Trust ~ data_demograph$gender)
```

Die Befragten machen sich leichte Sorgen um ihre Privatsphäre beim Verwenden von Empfehlungssystemen (`r describe.mean.sd(data_RS_trust$RS_Privacy_Trust) `). Insgesamt machen sich die Frauen etwas mehr Sorgen um ihre Privatsphäre (`r describe.mean.sd(m= tapply(data_RS_trust$RS_Privacy_Trust, data_demograph$gender, mean)[2], sd= tapply(data_RS_trust$RS_Privacy_Trust, data_demograph$gender, sd)[2])`) als die männlichen Befragten (`r describe.mean.sd(m= tapply(data_RS_trust$RS_Privacy_Trust, data_demograph$gender, mean)[1], sd= tapply(data_RS_trust$RS_Privacy_Trust, data_demograph$gender, sd)[1])`). Dieser Unterschied ist signifikant (`r describe.ttest(t.test(data_RS_trust$RS_Privacy_Trust ~ data_demograph$gender))`).
...
```{r Anlernen_describe}
describe.mean.sd(data_anlernen$Anlernen_ArticleReview_diversity)
describe.mean.sd(data_anlernen$Anlernen_ArticleReview_novelty)
describe.mean.sd(data_anlernen$Anlernen_ArticleReview_serendipity)
describe.mean.sd(data_anlernen$Anlernen_ArticleReview_relevance)
describe.mean.sd(data_anlernen$Anlernen_ArticleReview_quality)
...
## UBCF
```{r UBCF_describe}
describe.mean.sd(data_UBCF$UBCF_ArticleReview_diversity)
describe.mean.sd(data_UBCF$UBCF_ArticleReview_novelty)
describe.mean.sd(data_UBCF$UBCF_ArticleReview_serendipity)
describe.mean.sd(data_UBCF$UBCF_ArticleReview_quality)
...
## SVD
```{r SVD_describe}
describe.mean.sd(data_SVD$SVD_ArticleReview_diversity)
describe.mean.sd(data_SVD$SVD_ArticleReview_novelty)
describe.mean.sd(data_SVD$SVD_ArticleReview_serendipity)
describe.mean.sd(data_SVD$SVD_ArticleReview_relevance)
describe.mean.sd(data_SVD$SVD_ArticleReview_quality)
