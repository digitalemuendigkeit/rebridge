...
* `r table(data_demograph$gender)[2]/length(data_demograph$gender)*100` % weiblich
* `r table(data_demograph$gender)[1]/length(data_demograph$gender)*100` % männlich
* `r table(data_demograph$gender)[0]/length(data_demograph$gender)*100`0 % divers
...
* `r table(data_demograph$school)[1]/length(data_demograph$school)*100` % (noch) keine Schulabschluss
* `r table(data_demograph$school)[2]/length(data_demograph$school)*100` % einen Hauptschulabschluss
* `r table(data_demograph$school)[3]/length(data_demograph$school)*100` % einen Realschulabschluss 
* `r table(data_demograph$school)[4]/length(data_demograph$school)*100` % das (Fach-)Abitur

## Berufsausbildung
```{r}
data_demograph  %>% ggplot() + 
                    aes(x = factor(education), fill = factor(gender)) + 
                    geom_bar(position = "dodge") + 
                    scale_x_discrete(labels=c(`1` = "keine Ausbildung/Studium abgeschlossen", `2` = "Ausbildung", `3` = "Meister", `4` = "Hochschulabschluss", `5` = "Promotion", `6` = "Sonstiges")) +
                    ylab("Anzahl") + 
                    scale_fill_discrete(name="Geschlecht",labels=c("männlich", "weiblich")) +
                    coord_flip()
```
...
+ `r table(data_demograph$education)[1]/length(data_demograph$education)*100` % noch keine Ausbildung/ kein Studium abgeschlossen
+ `r table(data_demograph$school)[2]/length(data_demograph$school)*100` % eine Ausbildung abgeschlossen
+ `r table(data_demograph$school)[3]/length(data_demograph$school)*100` % einen Meister gemacht
+ `r table(data_demograph$school)[4]/length(data_demograph$school)*100` % einen Hochschulabschluss erworben
+ `r table(data_demograph$school)[5]/length(data_demograph$school)*100` % promoviert

## KUT
 Der KUT der Befragten ist mittelmäßig stark ausgeprägt: `r apastats::describe.mean.sd(data_kut$KUT)`
 
## Internetportale
Von den Befragten nutzen:

* `r table(data_newsConsum$newsPortal_1)/length(data_newsConsum$ResponseId)*100` % die Internetportale des öffentlich-rechtlichen Rundfunks
* `r table(data_newsConsum$newsPortal_2)/length(data_newsConsum$ResponseId)*100` % die Internetportale überregionaler Zeitungen
* `r table(data_newsConsum$newsPortal_3)/length(data_newsConsum$ResponseId)*100` % die Internetportale regionaler Zeitungen
* `r table(data_newsConsum$newsPortal_4)/length(data_newsConsum$ResponseId)*100` % Online-Nachrichtenportale
* `r table(data_newsConsum$newsPortal_5)/length(data_newsConsum$ResponseId)*100` % Suchmaschinen
* `r table(data_newsConsum$newsPortal_6)/length(data_newsConsum$ResponseId)*100` % Soziale Netwerke
* `r table(data_newsConsum$newsPortal_7)/length(data_newsConsum$ResponseId)*100` % sonstige Quellen
