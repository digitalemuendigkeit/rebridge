...
female <- table(data_demograph$gender)[2]r
...
data_demograph  %>% ggplot() + 
                    ...
                    ylab("Anzahl") + 
                    scale_fill_discrete(name="Geschlecht",labels=c("männlich", "weiblich")) +
                    coord_flip()Variablen f
```

Von den Befragten haben:

+ `r table(data_demograph$education)[1]/user*100` % noch keine Ausbildung/ kein Studium abgeschlossen
+ `r table(data_demograph$school)[2]/user*100` % eine Ausbildung abgeschlossen
+ `r table(data_demograph$school)[3]/user*100` % einen Meister gemacht
+ `r table(data_demograph$school)[4]/user*100` % einen Hochschulabschluss erworben
+ `r table(data_demograph$school)[5]/user*100` % promoviert
